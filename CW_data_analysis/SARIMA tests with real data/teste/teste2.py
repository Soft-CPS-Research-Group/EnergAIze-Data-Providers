    ...
    if np.isnan(df.iloc[i]['Value']):
        ...
        if x <= 6:
            ...
            for hour in daysAndHours:
                ...
                div = days // 2
                rest = days % 2
                print(f'{hour} , {days} , {div} , {rest} , {div+rest} , {div*2}')
                for j in range(div + rest):
                    nDays1 = div + rest + 1
                    value1 = df.loc[daysAndHours[hour][j] - pd.Timedelta(days=nDays1)]['Value']
                    while np.isnan(value1):
                        nDays1 += 1
                        value1 = df.loc[daysAndHours[hour][j] - pd.Timedelta(days=nDays1)]['Value']
                    forecast[daysAndHours[hour][j]] = value1

                for j in range(div, div*2+rest):
                    nDays1 = div*2+1
                    value1 = df.loc[daysAndHours[hour][j] - pd.Timedelta(days=nDays1)]['Value']
                    while np.isnan(value1):
                        nDays1 += 1
